...
var sortToggle = true;
    // api 호출 코드
    function callApi(code,month,page){
    $('#loading').show();
    $('#content').empty();
    $.ajax({
        method : "GET",
        ...
        datas = $(res).find('item');
        AddList(datas);
        endPage = Math.ceil($(res).find('totalCount').text() / 10);
        paging(parseInt(page),endPage);
        $('#loading').hide();
        $('#sort').show();
    });
}
...
// 거래 금액 순으로 정렬    
$(document).on('click',"#sort",sortPrice);
function sortPrice(){
    if(sortToggle){
        var sortedData = $(houseData).find('item').toArray().sort(function(a, b) {
            var priceA = parseInt($(a).find('거래금액').text().replace(/,/g, ''));
            var priceB = parseInt($(b).find('거래금액').text().replace(/,/g, ''));
...
$(document).on('click','.info-div',function(){
    ...
    '<div class = "detail-div" style = "padding-right:10px;">'+
        '<div>'+
            ...
            '<p class="info-p money">거래금액: '+$(data).find('거래금액').text()+'만원</p>'+
            '<p class="info-p address">도로명: '+$(data).find('도로명').text()+'</p>'+
...
$(document).on('click','.page',function(){
    ...
    window.parent.document.getElementById('mainiframe').src = 'houseSale.html?code=' + code + "&month=" + month +"&page=" + iValue;
})
...
$(document).on('click','#rtButton',function(){
    window.parent.document.getElementById('mainiframe').src = 'houseSale.html?code=' + code + "&month=" + month +"&page=" + endPage;
})
